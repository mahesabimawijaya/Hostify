      ...
      const fetchTransactions = async () => {
        try {
          const data = await fetchData(
            "api",
            `transaction/user/${loggedinUser.id}?productId=${productId}`
          );
      ...
      <div className="mx-auto max-w-screen-xl px-4 lg:px-12 pt-[10px]">
        <div className="flex items-center justify-between">
          <button
            ...
            className="px-5 py-2 bg-red-600 text-white rounded hover:opacity-80 lg:hidden"
          >
            Logout
          </button>
        </div>
        ...
        <h1 className="font-semibold text-xl mb-5 mt-14">
                ...
                {transactions.map((transaction, i) => (
                  <tr className="border-b " key={i}>
                    ...
                    <td className="px-4 py-3">
                      {transaction.term}{" "}
                      {transaction.term > 1 ? "months" : "month"}
                    </td>
                    <td className="px-4 py-3">
